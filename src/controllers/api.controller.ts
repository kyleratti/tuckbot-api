...
router.get("/debug/video/getall", (req, res) => {
  if (!authorized(req))
    return response(res, HttpStatus.FORBIDDEN, "Unauthorized");
...
router.get("/video/getmirrored", (req, res) => {
  ...
  Video.findAll({
    where: {
      [Op.or]: [{ status: Status.LocallyMirrored }]
    },
...
router.put("/video/queue", (req, res) => {
  ...
  Video.find({
    ...
    if (vid)
      return response(res, HttpStatus.CONFLICT, "Video already exists", {
        redditPostId: data.redditPostId,
...
router.post("/video/update", (req, res) => {
  if (!authorized(req))
    return response(res, HttpStatus.UNAUTHORIZED, "Unauthorized");
  ...
  if (data.filename) updatedData["filename"] = data.filename;

  if (data.views) updatedData["views"] = data.views;

  if (data.lastView) updatedData["lastView"] = data.lastView;
...
router.post("/video/upload", upload.single("video"), (req: any, res) => {
  ...
  if (configurator.file.local.storageDir) {
    Video.update(
      { status: Status.LocallyMirrored, filename: videoFile.originalname },
      { where: { redditPostId: redditPostId } }
    )
      .then(() => {
    ...
    return response(
      res,
      HttpStatus.INTERNAL_SERVER_ERROR,
      "File not not picked up by processor; request discarded"
    );
  }
});
