 ...
 */
function makeUrl(urlType, ...args) {
    return (configurator_1.default.app.environment === 'production' ? 'https' : 'http') + '://' + urlType + configurator_1.default.app.baseDomain + args.join('');
}
...
class ApiServer {
    constructor() {
        ...
        app.use('/video/upload', express_fileupload_1.default({
            safeFileNames: true,
            preserveExtension: 4,
            abortOnLimit: true
        }));
